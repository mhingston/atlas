...
/**
 * Reference workflow: code.pipeline.v1
 *
 * Runs harness assistance and then performs a review step.
 * Produces artifacts for both steps and sets needs_approval by default.
 */
export const codePipelineWorkflow: WorkflowPlugin = {
  id: "code.pipeline.v1",
  ...
  async run(ctx, input, jobId) {
    ...
    if (!goal) {
      throw new Error("code.pipeline.v1 requires 'goal' input");
    }
    ...
    logInfo("workflow.code_pipeline.run", {
      goal,
      mode,
    ...
    try {
      ...
      ctx.emitArtifact({
        ...
        job_id: jobId,
        title: `Code Pipeline: ${goal}`,
        content_md: result.summary,
        data: {
          schema_version: "1",
          produced_by: "code.pipeline.v1",
          harness_id: harnessId,
          mode: result.mode,
          goal,
        },
      ...
      logInfo("workflow.code_pipeline.complete", { recommendation });
    } catch (error) {
      logError("workflow.code_pipeline.failed", { error });
